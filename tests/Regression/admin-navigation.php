...
foreach (['/bp/visitors', '/bp/visitors/rsvps', '/bp/visitors/access', '/bp/admin/history'] as $href) {
    check('no link to an unbuilt page: ' . $href, !str_contains($allHtml, 'href="' . $href . '"'));
}
check('an unbuilt page is still placed in the map', Workspaces::locate('/admin/history') === ['workspace' => 'admin', 'page' => 'history', 'child' => null]);
